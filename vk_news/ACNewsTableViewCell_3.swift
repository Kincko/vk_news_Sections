...
//        postImage.sd_setImage(with: NSURL(string: model.postImage) as! URL, placeholderImage: #imageLiteral(resourceName: "no-thumbnail"))
//    
//        print("ширина картинки - \(postImage.image?.size.width) \nвысотка картинки - \(postImage.image?.size.height)")
//        
//        if postImage.image?.size.width != nil && postImage.image?.size.height != nil
//        {
//            let pictureRatio = frame.size.width / (postImage.image!.size.width / 0.5)
//            postImage.frame.size.width = frame.size.width
//            postImage.frame.size.height = postImage.image!.size.height * pictureRatio * 2
//
//            print("отношение сторон - \(pictureRatio)\nновая ширина - \(postImage.frame.size.width) \nновая высота - \(postImage.frame.size.height)\n")
//
//        }
    }
}
